...
## 🎓 Concepts cho sinh viên

### 1. Event Loop trong asyncio

```python
# asyncio event loop tự động quản lý:
async def handle_client(websocket):
    async for message in websocket:  # Await message
        await process(message)       # Await processing
        await broadcast(message)     # Await broadcast

# Khi await, event loop switch sang task khác → không lãng phí CPU
```
...
| Direction | Half-duplex (client → server) | Full-duplex (bidirectional) |
| Overhead | Header mỗi request (~500 bytes) | Frame header (~2-14 bytes) |
| Realtime | Polling/Long polling (inefficient) | True realtime push |

### 3. Concurrency trong Python

| Model | Suitable for | Example |
|-------|--------------|---------|
| **asyncio** | I/O-bound tasks | WebSocket, HTTP requests, DB queries |
| **threading** | I/O + shared memory | GUI applications |
| **multiprocessing** | CPU-bound tasks | Image processing, ML training |
...
1. **Thêm typing indicator:** Hiển thị "X đang gõ..." khi user đang nhập
2. **Private messages:** Cho phép gửi message riêng cho 1 user
3. **Chat rooms:** Tạo nhiều room, user join/leave room
4. **File upload:** Gửi file qua binary frames
5. **Authentication:** Thêm JWT token cho security
6. **Database:** Lưu chat history vào SQLite/PostgreSQL
7. **Notification:** Desktop notification khi có message mới
...
- **Môn học:** Lập trình mạng, Lập trình Web nâng cao
- **Thời lượng:** 2-3 buổi lab (4-6 tiếng)
- **Yêu cầu tiên quyết:** Python cơ bản, JavaScript cơ bản, HTTP protocol

**Cấu trúc bài giảng đề xuất:**
1. **Buổi 1:** Lý thuyết WebSocket, asyncio, demo chat
2. **Buổi 2:** Code along chat client/server
3. **Buổi 3:** Dashboard streaming, bài tập mở rộng
